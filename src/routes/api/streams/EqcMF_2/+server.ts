import { eqc2mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc2ClientAlert, WS_eqc2ClientProdData } from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc2mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc2mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc2mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc2ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc2ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc2mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc2mf_aggregatorCount.afternoonShiftCounting,
					night: eqc2mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc2ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc2ClientProdData.emitter.off('message', onProdDataHandler);
				}
