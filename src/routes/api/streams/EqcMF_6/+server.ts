import { eqc6mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc6ClientAlert, WS_eqc6ClientProdData} from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc6mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc6mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc6mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc6ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc6ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc6mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc6mf_aggregatorCount.afternoonShiftCounting,
					night: eqc6mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc6ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc6ClientProdData.emitter.off('message', onProdDataHandler);
				}
