import { eqc5mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc5ClientAlert, WS_eqc5ClientProdData } from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc5mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc5mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc5mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc5ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc5ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc5mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc5mf_aggregatorCount.afternoonShiftCounting,
					night: eqc5mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc5ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc5ClientProdData.emitter.off('message', onProdDataHandler);
				}
