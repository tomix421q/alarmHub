import { eqc4mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc4ClientAlert, WS_eqc4ClientProdData } from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc4mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc4mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc4mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc4ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc4ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc4mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc4mf_aggregatorCount.afternoonShiftCounting,
					night: eqc4mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc4ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc4ClientProdData.emitter.off('message', onProdDataHandler);
				}
