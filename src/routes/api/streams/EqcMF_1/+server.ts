import { eqc1mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc1ClientAlert, WS_eqc1ClientProdData } from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc1mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc1mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc1mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc1ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc1ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc1mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc1mf_aggregatorCount.afternoonShiftCounting,
					night: eqc1mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc1ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc1ClientProdData.emitter.off('message', onProdDataHandler);
				}
