import { eqc3mf_aggregatorCount } from '$lib/server/productionCount';
import { WS_eqc3ClientAlert, WS_eqc3ClientProdData } from '$lib/server/websocketClients';
...
interface ShiftCountData {
	morning: typeof eqc3mf_aggregatorCount.morningShiftCounting;
	afternoon: typeof eqc3mf_aggregatorCount.afternoonShiftCounting;
	night: typeof eqc3mf_aggregatorCount.nightShiftCounting;
	timestamp: string;
}
...
export const GET: RequestHandler = ({ request }) => {
	...
	const stream = new ReadableStream({
		start(controller) {
			...
			WS_eqc3ClientAlert.emitter.on('message', onAlertDataHandler);
			WS_eqc3ClientProdData.emitter.on('message', onProdDataHandler);

			// shift part count
			shiftCountInterval = setInterval(() => {
				const shiftData: ShiftCountData = {
					morning: eqc3mf_aggregatorCount.morningShiftCounting,
					afternoon: eqc3mf_aggregatorCount.afternoonShiftCounting,
					night: eqc3mf_aggregatorCount.nightShiftCounting,
			...
			request.signal.addEventListener('abort', () => {
				if (onAlertDataHandler) {
					WS_eqc3ClientAlert.emitter.off('message', onAlertDataHandler);
				}
				if (onProdDataHandler) {
					WS_eqc3ClientProdData.emitter.off('message', onProdDataHandler);
				}
