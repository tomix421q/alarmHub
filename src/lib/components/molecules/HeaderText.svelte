<script lang="ts">
	let { text, classNameh1 = '', animateOn = true, animateReloadSec = 300 } = $props();

	let effectLine1 = $state(false);
	let effectLine2 = $state(false);

	if (animateOn) {
		setInterval(() => {
			effectLine1 = !effectLine1;
			effectLine2 = !effectLine2;
		}, animateReloadSec * 1000);
	}
</script>
